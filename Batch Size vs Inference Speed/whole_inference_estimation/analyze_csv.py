"""
Analyzes the CSV produced by whole_inference_estimation.py.
...
Usage:
    python analyze_csv.py --csv output/ascending/fineweb_nsight_evaluation.csv

    # also average ascending+descending together (pools both files' rows,
    # so the average is weighted by each file's repeat count):
    python analyze_csv.py --csv output/ascending/fineweb_nsight_evaluation.csv \\
        --csv2 output/descending/fineweb_nsight_evaluation.csv
"""
...
def load(path):
    ...
    if missing:
        raise SystemExit(f"{path} is missing expected column(s): {sorted(missing)}. "
                          f"Is this really a whole_inference_estimation.py output CSV?")
    return df
...
def analyze(df, label):
    ...
    # it's directly comparable to whole_inference_estimation.py's own normalized_* columns and the
    # blog write-up's tables, which use the same B=1 basis.
    ...
    for _, row in ranked_total.iterrows():
        ...
        print(f"  batch_size={B:<3d}"
              ...
              f" | n_repeats={int(row['n_repeats'])}")

    print()
    print(f"Ranked fastest -> slowest, by pure forward (kernel, model forward only) time, "
...
if args.csv2:
    ...
    analyze(combined, f"COMBINED average (pools rows from both files above)")
