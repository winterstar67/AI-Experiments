...
# computes the val loss over eval_iters batches; the very first batch's forward+loss
# is also where we snapshot GPU memory (GPT is run in no_grad mode)
@torch.no_grad()
def estimate_loss():
    out = {}
    model.eval()
    ...
    for k in range(eval_iters):
        ...
        if k == 0:          # One snapshot
            ...
            torch.cuda.synchronize()
            torch.cuda.memory._dump_snapshot("out/first_batch_memory.pickle")
            torch.cuda.memory._record_memory_history(enabled=None)
            print_gpu_memory("estimate_loss k=0 (iteration)")
        else:
            logits, loss = model(X, Y)
        losses[k] = loss.item()
    out['val'] = losses.mean()
...
print("time for one loss computation",time.strftime("%H:%M:%S", time.gmtime(end-start)))
