...
config = {
    ...
    "seed": 1337,
    "device": "cuda",
    "dtype": 'float64', # 'bfloat16' if torch.cuda.is_available() and torch.cuda.is_bf16_supported() else 'float16',
    "compile": False,
                ...
                for _step in range(start_token_size, max_new_tokens):
                    ...
                    for layer_idx, block in enumerate(model.transformer.h):
                        if running_mode == "inf_no_cache":
                            K, V = block.attn.load_KV_activation(exist_KV_size)
                        elif running_mode == "inf_cache":
                            K, V = block.attn.load_KV(exist_KV_size)
                            expected_q_len = exist_KV_size if _step == start_token_size else 1
                            assert block.attn.last_q_len == expected_q_len, \
                                f"step {_step}, layer {layer_idx}: last_q_len={block.attn.last_q_len} != expected={expected_q_len}"
                        torch.save(
                                {
                                    "K_cache": K,
                                    "V_cache": V
                                },
                                os.path.join(KV_SAVE_DIR, f"{dtype}_{layer_idx}layer__{_step}token__{running_mode}.pt")
                            )
