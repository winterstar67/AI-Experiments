...
for _key in time_results.keys():
    ...
    for _step in range(start_token_size, max_new_tokens):
        list_for_mean = []
        for repeat in range(5, experiment_repeat_times):
            list_for_mean.append(
                time_results[_key][_step][repeat][0].elapsed_time(
                    time_results[_key][_step][repeat][1]
                )
            )
        mean_val = sum(list_for_mean) / len(list_for_mean)
        std_val = statistics.stdev(list_for_mean)
        time_results[_key][_step] = {"mean": mean_val, "std": std_val}
        print(_step, "Step time - mean:", mean_val, "std:", std_val)
...
torch.save(
        ...
        os.path.join(KV_SAVE_DIR, f"KVcache_every_token_result.pt")
    )
