...
counter = {"inf_no_cache": 0}

time_results = {"inf_no_cache": {}}
for _key in time_results.keys():
    for _step in range(1, 11):
        time_results[_key][2**_step] = []

with torch.no_grad():
    for running_mode in ["inf_no_cache"]:
        for start_token_size in range(1, 11):

            start_tokens = []
            for i in range(batch_size):
                start_tokens.append(
                    encoded_text[i * (2 ** start_token_size):(i + 1) * (2 ** start_token_size)]
                )
            ...
            for repeat_time in range(experiment_repeat_times):
                print(f"===== running_mode = {running_mode}, start_token_size = {start_token_size}, repeat = {repeat_time} =====")
                torch.manual_seed(seed)
                torch.cuda.manual_seed(seed)
                ...
                with ctx:
                    ...
                    end_evt.record()
                    time_results[running_mode][2 ** start_token_size].append((start_evt, end_evt))
                    ...
                    if counter['inf_no_cache'] <= 0:
                        decoded_next = [decode([idx_next[b, 0].item()]) for b in range(idx_next.size(0))]
                        print(f"[{running_mode}] start_token_size {start_token_size}: next tokens = {decoded_next}")
...
for _key in time_results.keys():
    ...
    for _step in range(1, 11):
        entries = time_results[_key][2**_step]
        if len(entries) > 0:
            list_for_mean = [s.elapsed_time(e) for (s, e) in entries[5:]]
            mean_val = sum(list_for_mean) / len(list_for_mean)
            std_val = statistics.stdev(list_for_mean)
            time_results[_key][2**_step] = {"mean": mean_val, "std": std_val}
            print(2**_step, "Step time - mean:", mean_val, "std:", std_val)
...
torch.save(
    ...
    os.path.join(KV_SAVE_DIR, f"No_KVcache_every_token_result.pt")
)