        ...
        for repeat_time in range(experiment_repeat_times):
            ...
            with ctx:
                ...
                for _step in range(start_token_size, max_new_tokens):
                    ...
                    #     record_shapes=True
                    # ) as prof:
                    print(f"{_step} Token process...")
                    logits, _ = model(idx_cond, running_mode=running_mode)
                    # if _step == max_new_tokens-1:
                    #     # 1) Basic table
                    #     print("========== 1) Basic table ==========")
                    #     print(prof.key_averages().table(sort_by="cuda_time_total", row_limit=20))

                    #     # 2) Group by shape
                    #     print("========== 2) Table groupd by shape ==========")
                    #     print(prof.key_averages(group_by_input_shape=True).table(
                    #         sort_by="cuda_time_total", row_limit=20
                    #     ))

                    # with open(f"{KV_SAVE_DIR}/{repeat_time}th_profile_result_in__{_step}_token__{running_mode}_mode.txt", "w") as f:
                    #     f.write(prof.key_averages().table(sort_by="cuda_time_total", row_limit=50))

                    # prof.export_chrome_trace(f"{KV_SAVE_DIR}/trace_in__{_step}_token__{running_mode}_mode.json")
