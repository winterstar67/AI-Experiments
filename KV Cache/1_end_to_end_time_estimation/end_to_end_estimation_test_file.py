    ...
    for running_mode in ["inf_no_cache", "inf_cache"]:
        for repeat_time in range(experiment_repeat_times):
            ...
            with ctx:
                ...
                print(f"[{running_mode}] {start_token_size} ~ {max_new_tokens} steps time:", round(end-start, 3), 's')
                time_results[running_mode].append(end-start)
            for layer_idx, block in enumerate(model.transformer.h):
                if running_mode == "inf_cache":
                    block.attn.reset_KV()
...
torch.save(
        ...
        os.path.join(KV_SAVE_DIR, f"KV_Cache_wall_time_estimation.pt")
    )